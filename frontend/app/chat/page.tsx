...
export default function DashboardPage() {
  ...
  // NEW — band_id migration: sessionId is now only used as owner_id to
  // find-or-create a band. All data (calendar/todos) is scoped by bandId.
  useEffect(() => {
    if (!sessionId) return;
    ...
    fetch(`${API}/band`, {
      ...
      .then((data) => {
        ...
        console.log("[chat/page] band_id ready:", data.band_id);
      })
      .catch((err) => console.error("[chat/page] Failed to get/create band", err));
  }, [sessionId]);
  ...
  // ── Load from Supabase — now scoped by bandId, not sessionId ────────────
  ...
  const loadFromSupabase = useCallback(async () => {
    ...
    try {
      ...
      console.error("[chat/page] Failed to load from Supabase", e);
    }
  }, [bandId, API]);
  ...
  useEffect(() => {
    ...
    console.log(`[chat/page] audio_features in localStorage for ${videoInfo.video_id}:`, stored ? "EXISTS" : "MISSING");
    if (stored) {
      try {
        ...
        console.error("[chat/page] Failed to parse audio features", e);
  ...
  const handleToggleTodo = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        console.error("[chat/page] Failed to update todo status", err);
      }
    },
    [todos, API]
  );
  ...
  const handleSaveContent = useCallback((eventId: number, content: string) => {
    ...
    }).catch((err) => console.error("[chat/page] Failed to save doc content", err));
  }, [API]);
  ...
  const handleDeleteEvent = useCallback(async (eventId: number) => {
    ...
    try {
      ...
      console.error("[chat/page] Failed to delete event", err);
    }
  }, []);
  ...
  const handleRescheduleEvent = useCallback(async (eventId: number, newDate: string) => {
    ...
    try {
      ...
      console.error("[chat/page] Failed to reschedule event", err);
    }
  }, []);
  ...
  const handleSendMessage = async (message: string) => {
    ...
    try {
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          if (toDelete) {
            ...
            console.warn("[chat/page] Delete matched no event for task:", taskTitle);
          }
        }
      } catch (_) {}
      ...
      console.error("[chat/page] Chat error:", err);
      setChatMessages((prev) => [
        ...prev,
        {
          role: "assistant",
  ...
  return (
    <div className="min-h-screen bg-[#f7f5f2] p-6">
      ...
      {/* Reset band data button — was /session/{sessionId}, now /band/{bandId} */}
      <button
        onClick={async () => {
